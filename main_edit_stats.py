...
import configuration_edit3 as cfg

# PCRaster itself
import pcraster as pcr
import pcraster.framework as pcrfw
#from pcraster.framework import pcrne #toegevoegd voor stats
...
class CatchmentModel(pcrfw.DynamicModel, pcrfw.MonteCarloModel):
  ...
  def dynamic(self):
    ...
    if cfg.with_shading:
      # 
      radiation (POTRAD, shading effect and inclination)
...
#probleem zit hem bij het voorvoegsel van die module enzo. Later weer nakijken    


    # discharge 
    # downstreamEdge=generalfunctions.edge(self.clone,2,0)
    # pcrfw.pits=pcrfw.pcrne(pcrfw.pit(self.d_runoffAccuthreshold.ldd),0)
    # outflowPoints=pcrfw.pcrand(downstreamEdge,pcrfw.pits)
    # totQ=pcrfw.ifthen(self.clone,pcrfw.maptotal(pcrfw.ifthenelse(outflowPoints,self.d_runoffAccuthreshold.RunoffCubicMetrePerHour,pcrfw.scalar(0))))

    # variable=totQ
    # variableSampled=pcrfw.ifthen(self.someLocs, variable)
...
################################
  ...
  def createInstancesInitial(self):
    ...
    if cfg.readDistributionOfParametersFromDisk:
      path = 'C:/Users/TEJan/Documents/Master/MasterThesis/pycatch-master'
